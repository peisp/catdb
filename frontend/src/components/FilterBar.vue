...
const whereKeywords = ['=', '!=', '<', '>', 'LIKE', 'IN', 'NOT', 'AND', 'OR', 'IS', 'NULL', 'BETWEEN']
const orderByKeywords = ['ASC', 'DESC']

function getWordAtCursor(input: string, cursorPos: number): { word: string; start: number; end: number } {
  // 找到光标所在位置的单词边界
  let start = cursorPos
...
function buildCompletions(input: string, cursorPos: number, mode: 'where' | 'orderBy'): string[] {
  const { word, start, end } = getWordAtCursor(input, cursorPos)
  if (word === '' && start !== end) return []

  const keywords = mode === 'where' ? whereKeywords : orderByKeywords
  const source = [...columnNames.value, ...keywords]

  // 根据前缀过滤
  const results = source.filter((item) => {
    const upper = item.toUpperCase()
    // 如果光标在单词开头，或者完整匹配前缀
    return upper.startsWith(word)
  })
...
function onInput(e: Event, mode: 'where' | 'orderBy') {
  ...
  showCompletions.value = items.length > 0 && items.length < 50
}
...
function onInputKeydown(e: KeyboardEvent, mode: 'where' | 'orderBy') {
  ...
  if (e.key === 'ArrowDown') {
    e.preventDefault()
    completionIndex.value = Math.min(completionIndex.value + 1, completions.value.length - 1)
  } else if (e.key === 'ArrowUp') {
    e.preventDefault()
    completionIndex.value = Math.max(completionIndex.value - 1, 0)
...
function applyCompletion(mode: 'where' | 'orderBy') {
  ...
  const ref = mode === 'where' ? whereInputRef : orderByInputRef
  const input = ref.value
  if (!input) return

  const cursorPos = input.selectionStart ?? input.value.length
  const { start, end } = getWordAtCursor(input.value, cursorPos)
...
// 重置：表切换时重置输入框（外部通过 key 或 watch 处理）
watch(
