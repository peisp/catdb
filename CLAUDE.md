...
11. **UI 规范以 `DESIGN.md` 为唯一来源**：写任何前端 UI 前必读 `DESIGN.md`，外观与交互（字体、字号、颜色、布局密度、控件风格、菜单/对话框形态等）一律以它为准，本文件不重复展开。
12. **数据库差异只落在两处**：针对特定数据库类型的行为/SQL 方言/quirk，只能写在 ① `dbdriver` 接口层的通用原语（由 `Capabilities`/`Dialect`/可选扩展接口如 `DatabaseRouter` 驱动，例：`NamespacedQuerier`）或 ② 对应驱动插件内部。`core`/`services`/`agent` 等通用层**禁止**出现 `if driver == "mysql"` 式分支或手写方言 SQL——发现通用层缺能力时，先在接口层补通用原语，再让驱动实现契约。目标：新数据库接入只需实现驱动契约，通用层零改动。
