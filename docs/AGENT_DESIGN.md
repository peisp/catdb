...
│ [连接名 ⛁][env 徽标]              [库 ▾][schema ▾]     │
│ ┌ 输入框（@表名 补全；随内容自动撑高到上限后内部滚动，   │
│ │  消息区与 dock 分界处的横向手柄拖拽调高，双击恢复自动） │
│ └────────────────────────────────────────────(↑/■)─┘ │
│ [Ask|Agent] [模型 ▾]                                  │
└─────────────────────────────────────────────────────┘
```
发送/停止是输入框右下角内嵌的圆形按钮：向上箭头 = 发送，方块 = 停止。

### 10.2 连接与库/schema 选择语义

- **连接在会话创建时选定，会话内不可更换**。理由：环境标签（闸 1）、方言提示词、会话授权、审计记录都锚定连接，中途换连接会让历史上下文误导模型且安全语义混乱。会话头提供「切换连接」快捷动作 = **新建会话**并带走当前输入草稿。
- **会话列表全局可见，不按连接过滤**（浏览层解绑，会话层绑定不变）：列表按 `updated_at` 倒序混排所有连接的会话，每项显示连接名 + 环境色标（prod 红色）。打开面板默认恢复最近一个会话；主界面切换连接不重置面板。
- **整页历史视图**：会话头的历史按钮把面板整体切换为历史页——顶栏「返回」+「清空历史」（二次确认；`ClearSessions` 级联删消息、取消运行中的 loop、审计保留，清空后自动新建默认会话），其下搜索框按标题/连接名过滤，列表每项显示标题、连接徽标与**最后更新时间**；重命名是**行内编辑**（铅笔图标进入，Enter/失焦提交、Esc 取消，空/未变视为取消，不弹窗），删除用垃圾桶图标走确认，点击条目载入该会话并返回对话页。
- **面板持有独立的连接上下文**，由**当前会话**的 `conn_id` 决定，与主界面选中连接（对象树/tab 区）解耦：切换会话只切换面板自身的连接上下文与环境徽标，不改动主界面。
- **懒连接**：消息历史存本地 SQLite，查看/切换会话**不建立数据库连接**；发送消息（引擎侧自行 connect）、展开库选择器、或输入 `@` 唤起表补全时才按需连接（补全菜单在加载期间显示「正在加载表清单」）。库列表在连接建立后加载；新会话 `current_db` 为空，命名空间加载成功后若仍未选库则默认取第一个库。
- **冷启动**（无任何会话时自动新建）的默认连接回退链：主界面当前连接 → 已打开的连接 → 列表第一个连接；一个连接都没有才显示空态。**新建会话**默认继承面板当前会话的连接，无当前会话时走同一回退链。
- **孤儿会话**（绑定的连接已被删除）：保留为只读归档——可查看历史、可删除，不能继续发送，列表与会话头标注「连接已删除」；不提供重新绑定。
- **库/schema 会话内可切换**（同连接内方言与环境标签不变）：选择器级联，schema 层按 `Capabilities.Schemas` 显隐；库选择器**遵循对象树的 schema 过滤**（与查询 tab 下拉一致，含树未挂载时读持久化选择的回退），会话已选中的库即使被过滤隐藏也保持显示。切换后更新系统提示词的默认命名空间与 `@` 补全范围，并在消息流插入一条系统提示线「上下文已切换到 x.y」；工具参数本就显式携带 `db`/`schema`（§4.2），历史消息不受影响。
...
- 输入 `@` 唤起补全，数据源为**当前选中库/schema** 的表清单（走 `api/metadata.ts` 既有缓存，输入防抖）；选中后 `@表名` **原样留在正文中**，语句读起来自然（「关联查询 @a 和 @b，通过 xxx 字段关联」）。
- 发送时从正文解析 `@` 提及（与已知表名做大小写不敏感匹配、去重），按命中的表拉取完整结构（列/索引/外键/注释）注入上下文（语义见 §4.3），该结构属于钉住内容不被压缩（§9）。在正文里编辑/删除提及即等于增删钉住的表。
- 消息气泡中的内联提及高亮显示。
...
ListSessions(ctx, connID string) ([]SessionInfo, error)    // connID 为空 = 返回全部会话（§10.2 全局列表）
...
ClearSessions(ctx) error                                  // 清空全部会话（§10.2 历史页；审计保留）
