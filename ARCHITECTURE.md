...
│  [MySQL✓] [MariaDB✓] [PostgreSQL✓] [SQLite✓] [DM✓] [...]  各实现 Driver 接口 │
└─────────────────────────────────────────────────────────────────┘
```
...
| MariaDB | go-sql-driver/mysql | 复用 mysqldrv（嵌入 `mysqldrv.Driver`） | 完全兼容 MySQL 协议；仅覆写驱动名与 editor dialect |
...
| 约束 | 缓解 |
|---|---|
| Wails v3 仍 beta，API 可能 break | 锁 `v3.0.0-beta.4`；wailsbridge/api 防腐层隔离 |
