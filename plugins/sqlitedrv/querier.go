...
// querier is the database/sql-backed Querier for SQLite. The same struct
// serves the pool (*sql.DB) and transactions (*sql.Tx).
type querier struct {
	exec execerQueryer
}
...
func (q *querier) Exec(ctx context.Context, sqlText string, args ...any) (dbdriver.ExecResult, error) {
	if q == nil || q.exec == nil {
		return dbdriver.ExecResult{}, fmt.Errorf("sqlitedrv: querier not initialized")
	}
...
func (q *querier) Query(ctx context.Context, sqlText string, args ...any) (dbdriver.ResultSet, error) {
	if q == nil || q.exec == nil {
		return nil, fmt.Errorf("sqlitedrv: querier not initialized")
	}
...
func (q *querier) Explain(ctx context.Context, sqlText string) (dbdriver.ResultSet, error) {
	return q.Query(ctx, "EXPLAIN QUERY PLAN "+sqlText)
}
